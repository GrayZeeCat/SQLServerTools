...
SELECT
    ...
    UPDATE STATISTICS ['' + SCHEMA_NAME([o].[schema_id]) + ''].['' + [o].[name] + ''] ['' + [s].[name] + '']
        WITH FULLSCAN'' + CASE WHEN [s].[no_recompute] = 1 THEN '', NORECOMPUTE'' ELSE '''' END + '', MAXDOP=' AS nvarchar(max)) + CAST(@maxDop AS nvarchar(10)) + CAST(';''
    , [o].[name]
    , [s].[name] AS [stat_name],
	[rowmodctr]
FROM (
            ...
            END ELSE 
                ...
                WHILE @@fetch_status >= 0
                    ...
                    IF ((@ind_rowmodctr is null) OR (@ind_rowmodctr <> 0))
                        ...
                        END
                        IF (len(@options) > 0) SELECT @options = @options + '', MAXDOP=' AS nvarchar(max)) + CAST(@maxDop AS nvarchar(10))+ CAST('''
								ELSE SELECT @options = ''MAXDOP=' AS nvarchar(max)) + CAST(@maxDop AS nvarchar(10)) + CAST('''
